...
def fast_reward_calculation_numba(current_v, current_t, target_time, punctuality_tolerance,
    ...
    if is_terminated:
        ...
        if time_error <= punctuality_tolerance:
            ...
            reward -= 60000.0 * excess_time_error

        # 停车奖励
        if current_v <= 1.0:
            reward += 1000000.0
        else:
            reward -= 10000.0 * abs(current_v - 1.0)
    else:
        # 进度奖励
        reward += 0.01 * delta_step

    # 超速惩罚
    if current_v > speed_limit_mps > 0:
        reward -= 100.0 * (current_v - speed_limit_mps)

    # 平稳性惩罚
    force_change = abs(control_force - previous_control_force)
    reward -= 1.0 * (force_change ** 2)
...
class HighSpeedTrainEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.observation_space = spaces.Box(
            ...
            high=np.array([1000000000, 1000000000, 1000000000, 100000000, 100000000, 100000000, 100000000,1000000000], dtype=np.float32),
            shape=(obs_dim,),
            dtype=np.float32
        )
    ...
    def _ultra_fast_action_mapping(self, action: np.ndarray, current_v: float) -> float:
        ...
        control_force = ((traction_force + braking_force) * 0.5) * action.item() + (traction_force - braking_force) * 0.5
        return control_force

    def _ultra_fast_observation(self) -> np.ndarray:
        """极速观测构建，使用预分配的数组"""
        obs = self._obs_template.copy()
        obs[0] = self.current_v_mps
        obs[1] = self.current_s_m
        obs[2] = self.left_s_m
        obs[3] = self.current_t_s
        obs[4] = self.target_time - self.current_t_s
        obs[5] = self.line_speed_limits[self._current_line_idx] * self._kmh_to_mps
        obs[6] = self.line_gradients[self._current_line_idx]
        obs[7] = self.line_curve_radius[self._current_line_idx]
        return obs
    ...
    def _calculate_reward(self, is_terminated: bool) -> float:
        ...
        if control_force < 0:
            ...
            if max_braking > 0:
                ...
                regen_bonus = 100 * (regen_ratio * self.regeneration_efficiency * self.transmission_efficiency) * self.delta_step
